...
use App\Models\back\cln_x_prev_str;
use App\Models\User;
...
class Cln_x_prev_strController extends Controller
{
    /**
    ...
    public function edit(Request $request)
    {
        ...
        $str = cln_x_prev_str::where('visit','=',$visit)->select('id','val')->get();

        return view('back.str.edit',compact('visit','id','str','doctor'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $doctor = $user->doctor;
        try {
            DB::table('cln_x_prev_str')->where('visit', '=', $request->visit)->delete();
            if ($request->str != null) {
                foreach ($request->str as $com1) {
                    if($com1 != null){
                        DB::insert('insert into cln_x_prev_str (visit ,patient,doc,val) values (?,?,?,?)', [$request->visit, $request->patient, $doctor->id, $com1]);
    ...
    public function destroy(Request $request)
    {
        try {
            DB::transaction(function () use ($request) {
                DB::table('cln_x_prev_str')->where('id', '=',$request->input)->delete();
