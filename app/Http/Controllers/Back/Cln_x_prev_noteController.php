...
use App\Models\back\cln_x_prev_not;
use App\Models\User;
...
class Cln_x_prev_noteController extends Controller
{
    /**
    ...
    public function edit(Request $request)
    {
        ...
        $note = cln_x_prev_not::where('visit','=',$visit)->select('id','val')->get();

        return view('back.note.edit',compact('visit','id','note','doctor'));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        $doctor = $user->doctor;
        try {
            DB::table('cln_x_prev_not')->where('visit', '=', $request->visit)->delete();
            if ($request->has('note')){
                foreach ($request->note as $com1) {
                    if($com1 != null){
                        DB::insert('insert into cln_x_prev_not (visit ,patient,doc,val) values (?,?,?,?)', [$request->visit, $request->patient, $doctor->id, $com1]);
